...
export default function blog({ posts }) {
  ...
  return (
    <section className='px-5 py-32 lg:px-0 blog-page bg-[url(/images/blog/bg.png)] bg-cover bg-top'>
        <div className="container px-4 mx-auto">
            ...
            <div className='flex flex-wrap justify-between'>
                         ...
                         {posts.map((post, index) => {
                                return (
                                    <div className='blog-item-card' key={index}>
                                        <Link href={`/blog/blog-detail/${post.slug}`} className='flex flex-col space-y-2 blog-item' >
                                            <Image src={post.frontmatter.images} alt='blog' width={400} height={300} className='object-cover object-center rounded-lg'/>
                ...
                <div className='order-1 hidden w-full gap-5 mb-10 flex-wrap: md:flex lg:flex-col lg:w-1/4 lg:order-2 sm:mb-0'>
                                ...
                                return (
                                    <li key={index}>
                                        <label htmlFor={`${item.id}`} className="inline-flex items-center gap-2">
                                            <input type="checkbox" id={`${item.id}`} className="bg-transparent border-gray-300 rounded"
                        ...
                        <ul className="px-4 space-y-4">
                            <li>
                                <div className='flex items-center justify-between gap-4'>
                                    <Image src='/images/blog/game1.png' alt='blog' width={100} height={100} className='object-cover object-center rounded-lg'/>
...
export async function getStaticProps() {
    // Get files from the posts dir (src/posts)
    const files = fs.readdirSync(path.join('src/posts')) // ['post1.md', 'post2.md']
